...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    public void restoreUrl(String shortUri, ServletRequest request, ServletResponse response) {
        ...
        // Does not contain in the cache, then check if it actually exist in the bloom filter
        ...
        // Then it checks if this is a verified null value to prevent cache penetration
        ...
        lock.lock();
        try {
            // code uses a double-checked locking pattern to handle potential race conditions when dealing with Redis and the locking mechanism
            ...
            //GOTO_IS_NULL_SHORT_LINK_KEY = "short-link:is-null:goto_%s";
            // code uses a double-checked locking pattern
            // Then it checks if this is a verified null value to prevent cache penetration
            ...
            ShortLinkGotoDO shortLinkGotoDO = shortLinkGotoMapper.selectOne(linkGotoQueryWrapper);
            // if null, add it to be a certified null value for 30 mins to prevent cache penetration
            if (shortLinkGotoDO == null) {
                stringRedisTemplate.opsForValue().set(String.format(GOTO_IS_NULL_SHORT_LINK_KEY, fullShortUrl), "-", 30, TimeUnit.MINUTES);
                ((HttpServletResponse) response).sendRedirect("/page/notfound");
                return;
            }
            // it has the redirect link in db
